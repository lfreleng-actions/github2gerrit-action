...
def apply_parameter_derivation(
    ...
    Derivation behavior:
    - Default: Automatic derivation enabled (G2G_ENABLE_DERIVATION=true by default)
    ...
    enable_derivation = os.getenv("G2G_ENABLE_DERIVATION", "true").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    if not enable_derivation:
        log.debug("Parameter derivation disabled. Set G2G_ENABLE_DERIVATION=true to enable automatic derivation.")
        return cfg
