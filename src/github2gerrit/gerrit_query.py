...
def query_changes_by_topic(
    ...
    *,
    ...
    try:
        changes = _execute_query_with_pagination(client, query, max_results=max_results)
        log.info(
        ...
        log.warning("Failed to query Gerrit changes for topic '%s': %s", topic, exc)
        return []
    ...
    while len(all_changes) < max_results:
        ...
        try:
            ...
            if not isinstance(response, list):
                log.warning("Unexpected Gerrit query response format: %s", type(response))
                break
...
def extract_pr_metadata_from_commit_message(commit_message: str) -> dict[str, str]:
    """
...
def validate_pr_metadata_match(
    ...
    for change in gerrit_changes:
        metadata = extract_pr_metadata_from_commit_message(change.commit_message)
        ...
        if github_hash and github_hash != expected_github_hash:
            log.debug(
                "Excluding change %s: GitHub-Hash mismatch (expected=%s, found=%s)",
                change.change_id,
    ...
    if len(validated_changes) != len(gerrit_changes):
        log.info(
            "Filtered Gerrit changes: %d -> %d (excluded %d due to metadata mismatch)",
            len(gerrit_changes),
