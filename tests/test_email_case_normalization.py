...
To ssh://testuser@gerrit.example.org:29418/test-project
 ! [remote rejected] HEAD -> refs/for/master%topic=GH-test-project-37,r=Test.User@Example.COM,r=master \
(Account 'Test.User@Example.COM' not found
error: failed to push some refs to 'ssh://testuser@gerrit.example.org:29418/test-project'
"""
    ...
    def test_extract_account_not_found_emails_multiple(self) -> None:
        ...
        error_output = """
 ! [remote rejected] HEAD -> refs/for/master (Account 'John.Doe@Example.COM' not found, \
...
"""
    ...
    def test_normalize_reviewer_emails_multiple_failed(self) -> None:
        ...
        reviewers = "Good@example.com,BAD1@test.COM,normal@site.org,BAD2@Company.NET"
        failed_emails = ["BAD1@test.COM", "BAD2@Company.NET"]

        result = self.orch._normalize_reviewer_emails(reviewers, failed_emails)
        expected = "Good@example.com,bad1@test.com,normal@site.org,bad2@company.net"

        assert result == expected

    def test_update_config_with_normalized_emails(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
"""
    ...
    def test_update_config_quoted_emails(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
"""
        ...
        original_emails = ["Test.User@Example.COM", "Another.User@Site.ORG", "Plain.Email@Domain.NET"]
        self.orch._update_config_with_normalized_emails(original_emails)
    ...
    def test_update_config_missing_organization(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_update_config_missing_file(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_update_config_no_matches(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
"""
        ...
        self.orch._update_config_with_normalized_emails(["notfound@example.com"])
    ...
    def test_integration_email_patterns(self) -> None:
        ...
        test_cases = [
            ...
            # Multiple accounts
            ("Account 'a@b.com' not found, Account 'c@d.org' not found", ["a@b.com", "c@d.org"]),
            # Mixed with other text
            ("Some error text Account 'test@site.net' not found more text", ["test@site.net"]),
            # Case variations
