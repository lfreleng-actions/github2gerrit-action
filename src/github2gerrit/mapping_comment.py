...
"""
Mapping comment utilities for serializing and deserializing Change-ID mapping blocks.
...
class ChangeIdMapping:
    """Represents a Change-ID mapping from a PR comment."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode not in ("multi-commit", "squash"):
            raise ValueError(f"{_MSG_INVALID_MODE}: {self.mode}")
        ...
        for cid in self.change_ids:
            if not cid.startswith("I") or len(cid) < 8:
                raise ValueError(f"{_MSG_INVALID_CHANGE_ID_FORMAT}: {cid}")
...
def _parse_mapping_block(block: str) -> ChangeIdMapping | None:
    ...
    # Validate required fields (github_hash is optional for backward compatibility)
    if not all([pr_url, mode, topic, change_ids]):
        log.debug(
            "Incomplete mapping block: pr_url=%s, mode=%s, topic=%s, change_ids=%d, github_hash=%s",
            bool(pr_url),
            mode,
            bool(topic),
            len(change_ids),
    ...
    try:
        return ChangeIdMapping(
            ...
            github_hash=github_hash or "",  # Default to empty string if not present
        )
...
def update_mapping_comment_body(
    ...
    updated_body = original_body[:start_idx] + new_block + original_body[end_idx:]
