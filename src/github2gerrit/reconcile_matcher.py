...
class ReconciliationMatcher:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            similarity_threshold: Minimum Jaccard similarity for subject matching
    ...
    def reconcile(
        ...
        remaining_commits = self._match_by_trailer(
            local_commits, gerrit_changes, used_changes, matches, strategy_counts
        )

        # Pass B: Exact subject matching
        remaining_commits = self._match_by_subject_exact(
            remaining_commits, gerrit_changes, used_changes, matches, strategy_counts
        )
        ...
        orphaned = [change for change in gerrit_changes if change.change_id not in used_changes]
    ...
    def _match_by_trailer(
        ...
        for commit in commits:
            ...
            if cid in gerrit_by_id and cid not in used_changes:
                ...
                strategy_counts[MatchStrategy.TRAILER] = strategy_counts.get(MatchStrategy.TRAILER, 0) + 1
                log.debug("Trailer match: %s -> %s", commit.sha[:8], cid)
            else:
                # Change-ID not found in Gerrit or already used
                remaining.append(commit)
                if cid in used_changes:
                    log.warning("Duplicate Change-ID %s in commit %s (already used)", cid, commit.sha[:8])
    ...
    def _match_by_subject_exact(
        ...
        for commit in commits:
            ...
            # Use first available candidate (could enhance with additional criteria)
            ...
            strategy_counts[MatchStrategy.SUBJECT_EXACT] = strategy_counts.get(MatchStrategy.SUBJECT_EXACT, 0) + 1
            ...
            log.debug("Subject exact match: %s -> %s", commit.sha[:8], gerrit_change.change_id)
    ...
    def _match_by_file_signature(
        ...
        for commit in commits:
            ...
            used_changes.add(gerrit_change.change_id)
            strategy_counts[MatchStrategy.FILE_SIGNATURE] = strategy_counts.get(MatchStrategy.FILE_SIGNATURE, 0) + 1
            ...
            log.debug("File signature match: %s -> %s (sig=%s)", commit.sha[:8], gerrit_change.change_id, file_sig)
    ...
    def _match_by_subject_similarity(
        ...
        available_changes = [change for change in gerrit_changes if change.change_id not in used_changes]
        ...
        for commit in commits:
            ...
            for change in available_changes:
                ...
                similarity = compute_jaccard_similarity(commit_tokens, change_tokens)
                if similarity >= self.similarity_threshold and (best_match is None or similarity > best_match[1]):
            ...
            if best_match:
                ...
                log.debug(
                    "Similarity match: %s -> %s (confidence=%.2f)", commit.sha[:8], gerrit_change.change_id, confidence
                )
            else:
                remaining.append(commit)
    ...
    def _log_reconciliation_summary(self, result: ReconciliationResult) -> None:
        ...
        if result.strategy_counts:
            ...
            log.info("Matching strategies: %s", " ".join(strategy_details))

        # Orphaned changes warning
        if result.orphaned_changes:
            orphaned_ids = [change.change_id for change in result.orphaned_changes]
            log.warning(
        ...
        debug_data = {
            ...
            "strategies": {strategy.value: count for strategy, count in result.strategy_counts.items()},
        }
        log.debug("RECONCILE_SUMMARY json=%s", json.dumps(debug_data))
...
def create_local_commit(
    ...
    existing_change_id = existing_change_ids[-1] if existing_change_ids else None
