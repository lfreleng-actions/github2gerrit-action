...
def main(
    ...
    reuse_strategy: str = typer.Option(
        ...
        help="Strategy for reusing Change-IDs: topic, comment, topic+comment, none.",
    ),
    ...
    if os.getenv("PERSIST_SINGLE_MAPPING_COMMENT"):
        persist_single_mapping_comment = parse_bool_env(os.getenv("PERSIST_SINGLE_MAPPING_COMMENT"))
    ...
    os.environ["ALLOW_ORPHAN_CHANGES"] = "true" if allow_orphan_changes else "false"
    os.environ["PERSIST_SINGLE_MAPPING_COMMENT"] = "true" if persist_single_mapping_comment else "false"
...
def _build_inputs_from_env() -> Inputs:
    return Inputs(
        ...
        persist_single_mapping_comment=env_bool("PERSIST_SINGLE_MAPPING_COMMENT", True),
