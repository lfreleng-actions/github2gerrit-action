...
def test_apply_pr_title_body_preserves_change_id_footer(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Assert
    assert amended_messages, "Expected at least one amend call capturing a message"

    # Use the first amend call which should have the PR title/body and preserved trailers
    ...
    assert any(ln.startswith("GitHub-Hash: ") for ln in lines), "GitHub-Hash trailer missing after amend"
    assert lines[-1].startswith("GitHub-Hash: "), "GitHub-Hash must be the last trailer line in the footer"
    ...
    # Ensure proper trailer ordering: Signed-off-by, Change-Id, GitHub-PR, GitHub-Hash
    signed_idx = None
    change_idx = None
    github_pr_idx = None
