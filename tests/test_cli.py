...
def test_validation_local_cli_requires_derivation_disabled(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that local CLI usage fails validation when G2G_ENABLE_DERIVATION=false is set."""
    # Clear environment first
    for key in list(os.environ.keys()):
        monkeypatch.delenv(key, raising=False)
