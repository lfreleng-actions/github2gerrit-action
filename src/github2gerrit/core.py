...
from .gerrit_query import validate_pr_metadata_match
...
from .reconcile_matcher import ReconciliationMatcher
...
class Orchestrator:
    ...
    # Phase 3 introduces reconciliation helpers below for reusing prior Change-Ids
    ...
    def _emit_change_id_map_comment(
        ...
        *,
        ...
        try:
            ...
            pr_url = f"{gh_context.server_url}/{gh_context.repository}/pull/{gh_context.pr_number}"
            ...
            if replace_existing:
                ...
                comment_indices = find_mapping_comments([c.body or "" for c in comments])
                if comment_indices:
                    ...
                    updated_body = update_mapping_comment_body(body, new_mapping)
                    latest_comment.edit(updated_body)  # type: ignore[attr-defined]
                    log.debug("Updated existing mapping comment for PR #%s", gh_context.pr_number)
                    return
    ...
    def _perform_robust_reconciliation(
        ...
        """
        Perform robust multi-pass reconciliation of local commits with Gerrit changes.

        Args:
            inputs: Configuration inputs
            gh: GitHub context
            gerrit: Gerrit connection info
            local_commits: List of local commits to reconcile

        Returns:
            Ordered list of Change-IDs (reused + newly generated)
        """
        if not local_commits:
            return []

        strategy = inputs.reuse_strategy.lower()
        if strategy == "none":
            log.info("Reconciliation disabled by configuration")
            return []

        log.info(
            "Starting robust reconciliation with strategy '%s' for %d local commits",
            strategy,
            len(local_commits),
        )

        # Build expected PR metadata for validation
        ...
        all_gerrit_changes: list[GerritChange] = []

        # Strategy: topic-based discovery
        if "topic" in strategy:
            topic = f"GH-{gh.repository_owner}-{gh.repository.split('/')[-1]}-{gh.pr_number}"
            try:
                from .gerrit_rest import GerritRestClient

                client = GerritRestClient(
                    base_url=f"https://{gerrit.host}:{gerrit.port}",
                    auth=None,  # Will use SSH for actual operations
                )

                gerrit_changes = query_changes_by_topic(
                    client,
                    topic,
                    statuses=["NEW", "MERGED"] if not inputs.allow_orphan_changes else ["NEW"],
                )

                # Validate metadata to prevent cross-PR contamination
                validated_changes = validate_pr_metadata_match(
                    gerrit_changes,
                    expected_pr_url,
                    expected_github_hash,
                )

                all_gerrit_changes.extend(validated_changes)
                log.info("Found %d validated Gerrit changes via topic query", len(validated_changes))

            except Exception as exc:
                log.debug("Topic-based discovery failed: %s", exc)

        # Strategy: comment-based fallback
        if "comment" in strategy and not all_gerrit_changes:
            try:
                from .github_api import build_client
                from .github_api import get_pull
                from .github_api import get_repo_from_env
                ...
                issue = pr_obj.as_issue()
                comments = list(issue.get_comments())
                comment_bodies = [getattr(c, "body", "") or "" for c in comments]

                mapping = parse_mapping_comments(comment_bodies)
                if mapping and validate_mapping_consistency(mapping, expected_pr_url, expected_github_hash):
                    # Convert mapping to simple reuse by index (legacy behavior)
                    log.info(
                        "Using comment-based reconciliation: %d Change-IDs from mapping",
                        len(mapping.change_ids),
                    )
                    # Simple index-based reuse for comment fallback
                    result_ids = []
                    for i, commit in enumerate(local_commits):
                        if i < len(mapping.change_ids):
                            result_ids.append(mapping.change_ids[i])
                        else:
                            # Generate new Change-ID for additional commits
                            import hashlib
                            import time

                            content = f"{time.time()}_{commit.sha}"
                            hash_obj = hashlib.sha256(content.encode("utf-8"))
                            result_ids.append("I" + hash_obj.hexdigest()[:40])
        ...
        # Perform multi-pass matching if we have Gerrit changes
        if all_gerrit_changes:
            matcher = ReconciliationMatcher(
                similarity_threshold=inputs.similarity_subject,
                allow_duplicate_subjects=True,
            )

            reconciliation_result = matcher.reconcile(local_commits, all_gerrit_changes)

            # Log structured summary if enabled
            if inputs.log_reconcile_json:
                import json

                summary_data = {
                    "total_local": len(local_commits),
                    "reused": reconciliation_result.reused_count,
                    "new": reconciliation_result.new_count,
                    "orphaned": len(reconciliation_result.orphaned_changes),
                    "strategy": strategy,
                }
                log.info("RECONCILE_SUMMARY json=%s", json.dumps(summary_data))

            return reconciliation_result.change_ids

        # Fallback: generate all new Change-IDs
        log.info("No existing changes found, generating new Change-IDs")
        new_ids = []
        for commit in local_commits:
            ...
            content = f"{time.time()}_{commit.sha}"
            hash_obj = hashlib.sha256(content.encode("utf-8"))
            new_ids.append("I" + hash_obj.hexdigest()[:40])
    ...
    def _extract_local_commits_for_reconciliation(
        ...
        Returns:
            List of LocalCommit objects representing local commits to be submitted
        """
        ...
        for index, commit_sha in enumerate(commit_list):
            try:
                ...
                log.warning("Failed to extract commit info for %s: %s", commit_sha[:8], exc)
                continue

        log.info("Extracted %d local commits for reconciliation", len(local_commits))
        return local_commits

    def _attempt_change_id_reconciliation(self, gh: GitHubContext) -> list[str]:
        """
        Legacy reconciliation method - kept for backward compatibility.

        This is now a thin wrapper around the robust reconciliation system.
        """
        try:
            if not gh.pr_number:
                return []
            ...
            mapping = parse_mapping_comments(comment_bodies)
            if mapping:
                log.debug("Recovered %d prior Change-Id(s) for reuse", len(mapping.change_ids))
                return mapping.change_ids
            else:
                return []

        except Exception as exc:
            log.debug("Legacy Change-Id reconciliation skipped: %s", exc)
            return []

    def _parse_previous_change_id_map(self, bodies: list[str]) -> list[str]:
        """
        Legacy parsing method - now uses mapping_comment utility.
        """
        mapping = parse_mapping_comments(bodies)
        return mapping.change_ids if mapping else []
    ...
    def execute(
        ...
        if inputs.submit_single_commits:
            ...
            local_commits = self._extract_local_commits_for_reconciliation(inputs, gh)
            reuse_ids = self._perform_robust_reconciliation(inputs, gh, gerrit, local_commits)

            if reuse_ids:
                try:
                    prep = self._prepare_single_commits(inputs, gh, gerrit, reuse_change_ids=reuse_ids)
                except Exception as exc:
                    log.debug("Re-preparation with reuse Change-Ids failed (continuing without reuse): %s", exc)
        else:
            # For squash mode, use simplified reconciliation
            reuse_ids = self._attempt_change_id_reconciliation(gh)
            if reuse_ids:
                try:
                    prep = self._prepare_squashed_commit(inputs, gh, gerrit, reuse_change_ids=reuse_ids[:1])
                except Exception as exc:
                    log.debug("Re-preparation with reuse Change-Ids failed (continuing without reuse): %s", exc)
    ...
    def _prepare_single_commits(
        ...
        for idx, csha in enumerate(commit_list):
            ...
            try:
                ...
                log.debug("Skipping metadata trailer injection for commit %s: %s", csha, meta_exc)
    ...
    def _prepare_squashed_commit(
        ...
        # Phase 3: if external reuse list provided, override with first Change-Id
        if reuse_change_ids:
        ...
        # Phase 1: ensure metadata trailers before creating commit (idempotent merge)
        try:
            meta = self._build_pr_metadata_trailers(gh)
            if meta:
            ...
            log.debug("Skipping metadata trailer injection (squash path): %s", meta_exc)
    ...
    def _apply_pr_title_body_if_requested(
        ...
        try:
            trailers_after = git_last_commit_trailers(keys=["Change-Id"], cwd=self.workspace)
            self._latest_apply_pr_change_ids = trailers_after.get("Change-Id", [])
        except Exception as exc:
            log.debug("Failed to collect Change-Ids after apply_pr_title: %s", exc)
        # Phase 1: ensure trailers present even if earlier logic skipped (idempotent)
        try:
            meta = self._build_pr_metadata_trailers(gh)
            if meta:
                cur_msg = run_cmd(
            ...
            log.debug("Skipping post-apply metadata trailer ensure: %s", meta_exc)
    ...
    def _push_to_gerrit(
        ...
        *,
        ...
        try:
            ...
            # Check for account not found error and try with case-normalized emails
            account_not_found_emails = self._extract_account_not_found_emails(exc)
            if account_not_found_emails:
                normalized_reviewers = self._normalize_reviewer_emails(reviewers, account_not_found_emails)
                if normalized_reviewers != reviewers:
                    log.info("Retrying with case-normalized email addresses...")
                    try:
                        ...
                        retry_args = [
                            retry_args[i] for i in range(len(retry_args)) if i == 0 or retry_args[i - 1] != "--reviewer"
                        ]
                        ...
                        log.debug("Retrying git review command with normalized emails: %s", " ".join(retry_args))
                        run_cmd(retry_args, cwd=self.workspace, env=env)
                        log.info("Successfully pushed changes to Gerrit with normalized email addresses")
                        ...
                        log.warning(
                            "Retry with normalized emails also failed: %s", self._analyze_gerrit_push_failure(retry_exc)
                        )
                        ...
                        try:
                            ...
                            log.debug("Failed to emit Change-Id map comment (retry path): %s", cexc)
                        return
        ...
        # Cleanup temporary branch used during preparation
            ...
            try:
                ...
                log.debug("Failed to emit Change-Id map comment (success path): %s", exc_emit)
        # Cleanup temporary branch used during preparation
    ...
    def _extract_account_not_found_emails(self, exc: CommandError) -> list[str]:
        ...
        valid_emails = [email for email in matches if re.match(email_pattern, email)]
    ...
    def _normalize_reviewer_emails(self, reviewers: str, failed_emails: list[str]) -> str:
        ...
        for reviewer in reviewer_list:
            if reviewer in failed_emails:
                ...
                if normalized != reviewer:
                    log.info("Normalizing email case: %s -> %s", reviewer, normalized)
    ...
    def _update_config_with_normalized_emails(self, original_emails: list[str]) -> None:
        """Update configuration file with normalized email addresses.
        ...
        try:
            ...
            org = os.getenv("ORGANIZATION") or os.getenv("GITHUB_REPOSITORY_OWNER")
            if not org:
            ...
            if not config_path_obj.exists():
                log.debug("Config file does not exist, skipping update: %s", config_path_obj)
                return
                ...
                if normalized_email != original_email:
                    ...
                    for pattern in patterns:
                        if pattern in content:
                            replacement = pattern.replace(original_email, normalized_email)
                            content = content.replace(pattern, replacement)
                            log.info("Updated config file: %s -> %s", pattern, replacement)

            # Write back if changes were made
            if content != original_content:
                config_path_obj.write_text(content, encoding="utf-8")
                log.info("Configuration file updated with normalized email addresses: %s", config_path_obj)
            else:
                log.debug("No email addresses found in config file to normalize")

        except Exception as exc:
            log.warning("Failed to update configuration file with normalized emails: %s", exc)
